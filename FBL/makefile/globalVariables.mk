...
# If we use the folder structure for the object files then a problem results if we need to
# reference some files in folders, which are not sub-ordinated, but, e.g., a sibling,
# uncle, nephew, etc., of the current project directory. If we use the relative path
# operator .. to address to those folders then the object files will use the same path
# designation and end up outside the desired bin folder. This is tackled by specifying an
# additional folder fragment, which is put between the target folder and all object file
# paths.
#   The inserted path fragment makes if difficult for make to anticipate, where to find the
# source file that relates to a required object file. To overcome this, we need to make
# the common parent of all addressed folder tree an anchor for searching sources. This
# parent is addressed by relative path from the project root directory.
ifeq ($(useFlatObjectFileStructure),)
    commonParentOfAllSourcePaths  = ../
    pathFromCommonParentToProject = FBL/
endif
