...
This repository contains a CAN based flash bootloader (FBL) for the
...
- If you use another board with the MPC5748G, then you'd probably need to
  ...
  (for SPI connected devices) and the pin configuration for the chosen CAN
  device.
...
The FBL is an embedded application for the MPC5748G. It has a CAN device
driver and a flash ROM driver. It integrates elements from the Open Source
CAN stack https://github.com/PeterVranken/comFramework/wiki[comFramework^]
to implement the
...
messages, the CRO command message and the DTO response message. The FBL
requires the configuration of the CAN device to use and the two CAN IDs
for CRO and DTO. The choice is made at compile-time by means of `#define`
switches in file `code/system/CCP/ccp_taskCcp.h`.
...
The configuration setting is found as macro `CCP_IDX_CAN_BUS_FOR_CCP` in
...
then you first need to modify the CAN driver configuration and enable and
configure that device properly.

The CAN IDs are set by means of `#define` switches in file
`code/system/CCP/ccp_taskCcp.h`:
...
You can choose any ID and switch between 11 and 29 Bit IDs.
...
The public key is a compile-time configuration setting of the FBL. It is
configured as a constant array in file
...
key pair. The flash tool has an option to generated new, randomly chosen
key pairs. Any other key pair generator for Ed25519 can of course be used,
too.

There is no compile-time switch in the FBL to disable the authentication
procedure.
...
|UPLOAD |0x04 | Argument is a count in the range 1..5. This number of
bytes is read at memory address `MTA0` and returned in the response. MTA
is incremented by the requested number of bytes.
...
erase, starting at flash ROM address `MTA0`. Effectively, the command can
...
flash ROM is erased then this takes about 25 s. The CCP client needs to
have an according timeout, when submitting this command.

|PROGRAM |0x18 | Argument is a number of data bytes in the range 1..5.
These bytes are programmed at flash ROM address `MTA0`. The MTA is
...
bytes. The response contains the size of the function result in Byte.
`MTA0` is set to the upload address for the function result. The FBL
supports these custom functions:
...
and will properly respond to a new CCP `CONNECT` command addressed to its
station address.
|=======
...
- None of the commands will succeed if the authentication procedure hasn't
  been completed before. The authentication is valid as long as the
  session is alive, i.e., until an explicit `DISCONNECT` or until an error
  occurs.
- `CLEAR_MEMORY`, `UPLOAD`, `DOWNLOAD`, `DOWNLOAD_6`, `PROGRAM` and
  `PROGRAM_6` do all depend on `MTA0`, normally set by an initial
  `SET_MTA`.
...
- The organization of the flash ROM in so-called "quad-pages" of 128 Byte
  ...
  `PROGRAM_6` commands fail with high likelihood if the CCP doesn't send
  the data in order of monotonic raising addresses.
...
- The FBL has a session timeout of 5 s. The CCP client must not wait
  longer than 5 s to submit a CCP command after it got the response from
  the previous one.
...
the failing CCP command.

Which doesn't really matter in practice; whenever a command fails, the
intended procedure -- be it a verify, erasure or application programming
-- has failed and the session is closed and the CCP client can try to
repeat the entire procedure. Continuation of data flashing after error
analysis would anyway be unlikely to succeed due to the characteristics of
the flash ROM.

Asynchronous function execution mainly relates to programming: When the
FBL receives a `PROGRAM` or `PROGRAM_6` command, it will first buffer the
data and only later do the actual flash programming.
...
Before a page can be programmed again, the entire flash block containing
the page needs to be erased again -- and this will erase many other pages
at the same time. (For the MPC5748G, a flash block consists of 2048
pages.)
...
Writing a page is triggered implicitly as soon as a CCP `PROGRAM` or
`PROGRAM_6` command addresses a byte belonging into another quad-page.
Given this strategy and considering that re-programming a quad page is not
possible, the easiest way to safely write all bytes of all quad-pages is
sending them in order of raising addresses.

A back and forth between programmed addresses is not impossible but would
require deep understanding of the buffering strategy and it would require
to strictly consider the quad-page boundaries.

== Verify

Each page write is immediately and unconditionally followed by a verify of
the written data. The page write buffer is released for re-use only after
completion of the verify operation. A mismatch between page contents and
data buffer is reported asynchronously as negative response from the next
CCP `PROGRAM` or `PROGRAM_6` command.

With respect to potential physical flash programming errors, it is not
necessary for a flash tool to upload all data after programming for a
verify. Such a second phase in the programming procedure doubles the
overall execution time (data transmission, not flash programming is the
bottle-neck); the only added value is the verification of the error free
CAN transmission.
...
(Windows or Linux) provides a virtual COM device.

Open a terminal application on your PC, which the evaluation board is
connected to via USB. The NXP software implements a virtual COM device on
the PC, which is called "OpenSDA - CDC Serial Port". In case of doubts,
use the Windows GUI "Computer Management", tab `System Tools/Device
Manager` to find out, which one it is.

In the terminal application, open this COM device with 115200 Bd, 8 Bit,
no parity bit, 1 stop bit. End of line, for in- and output, is the
...
In this case, the FBL checks the memories for a flashed, valid
application. If it finds a valid boot header then this header contains a
time designation. The FBL starts entirely up, opens the CAN device and
listens for potential CCP `CONNECT` commands at its station address. A CCP
client has the opportunity to send such a command and to begin some CCP
communication, e.g., one of those typical command sequences explained in
<<secCcpCmdSeqs,above>>.
+
...
zero. In this case, reset 1 behaves like reset 2 and the startup time for
a flashed, valid application is significantly shorter; it is nearly
identical to the use of the application without FBL. Please note that the
application needs to support reset 3 in this case; otherwise the FBL would
become useless.
+
...
+
In this case, the FBL checks the memories for a flashed, valid
application. If it finds a valid boot header then this header contains the
start address of the application. The FBL launches the application by
direct jump to that address.
+
Launching by jump instruction is possible because at this time the FBL has
...
the CAN device and listens for CCP `CONNECT` commands -- this time
forever, without any time limits.

3. *The reset to launch the FBL.* This is a SW reset, normally triggered
by the flashed application.
+
An application made for use with the FBL can implement some communication
with its user, that allows the user to start the FBL. A typical
application function could be titled "Update firmware" or similar. It
could be triggered by CAN or by a specific high-low combination of GPIO
pins, or by any other suitable mechanism.
+
...
this feature needs to implement the same or an equivalent mechanism as the
FBL for triggering the reset; see next section for details.

Most of the time, during normal use of a DEVKIT-MPC5748G application,
reset 1 is used. The board is powered, the application is launched and the
user experiences the expected behavior of board and application.
...
communicate with the application, let it demand reset 3, and can then
start the CCP communication with the meanwhile started FBL for
re-programming the application. The entire process "Update firmware"
appears to the user as a simple and seamlessy operation.
...
The FBL specific aspect is a command flag, which is written into RAM at a
...
3. Write the command flag into the next 32 Bit address, i.e., at
   0x40000004. The command flag to go into and remain in the FBL after
   reset is 0xADeafBee.
...
manipulation in the 2nd and 3rd step can have any unforeseen harmful
effect if some other code entity would still execute. An alternative would
be ensuring by memory partitioning that the first 8 Byte of RAM are never
and nowhere used in the application.
...
environment on the development PC. In particular, the location of the GNU
...
project root, to configure the environment. This script requires
customization prior to its first use. Windows users open it in a text
editor and follow the instructions marked by TODO tags. Mainly, this is
about specifying the installation directory of GCC.

Non-Windows users can read this script to see which (few) environmental
...
it in a text editor and follow the instructions that are marked by TODO
tags. Mainly, this is about specifying the installation directory of the
S32 Design Studio.
...
other than by executing the script `S32DS-IDE.ps1` or its equivalent on
non-Windows PCs.
...
to produce the flashable files `bin\ppc\DEBUG\DEVKIT-MPC5748G-FBL.elf`,
and `bin\ppc\PRODUCTION\DEVKIT-MPC5748G-FBL.elf`.
...
in RAM. (See next section for details). With option `LINK_IN_RAM`, the
same commands:

    mingw32-make -sO build
    mingw32-make -sO build CONFIG=PRODUCTION

produce the flashable files `bin\ppc\DEBUG-RAM\DEVKIT-MPC5748G-FBL.elf`,
and `bin\ppc\PRODUCTION-RAM\DEVKIT-MPC5748G-FBL.elf`.
...
Cygwin port will fail with obscure and misleading error messages. For your
convenience, an appropriate recent version of the MinGW make processor has
been uploaded into this GitHub project. The PowerShell startup script
...
and data objects in the binary file (`*.elf`). It will erase and flash the
ROM if the objects have ROM addresses and it will load them into RAM if
the objects are located in RAM. So all that needs to be done is defining
the memory addresses in the linker scripts accordingly.

Under control of a macro in the main makefile, `GNUmakefile`, the linker
chooses different address ranges. If the macro `LINK_IN_RAM` is an element
of the list of macros then the linker will divide the physically available
...
there are barely any dependencies. The MPU configuration is the principal
...
preliminary and temporary way of working only. Running the software can be
...
after a power-up or without a connected Design Studio is impossible.

=== Flashing and debugging

The binary code of the compiled FBL can be flashed and debugged with the
S32 Design Studio IDE. Effectively, flashing means to start the GNU
debugger (GDB) and to let it "load" the `*.elf` file. If the code is
linked in flash ROM address space then this loading operation means
writing to the flash. Consequently, a flash configuration in the Eclipse
IDE is nearly identical to any ordinary debug configuration; just the
option "Load executable" to load a file is checked. Ordinary debug
configurations, i.e., for debugging, don't have this check mark set:
...
the focus, to filter all configurations intended for flashing and select
the one you need. Press the Enter key or click on the button "Debug" and
the flash process begins. Progress and status messages are printed in one
of the console windows in the lower right pane.

It is a bit counter-intuitive that flashing with GDB is just a kind of
side effect of starting the debugger. Rather than ending with a message
like "Congratulations, flashing successfully completed", the flashing
process ends with a ready-to-use interactive debug session: The source
code window shows the startup code for the boot core Z4A and you could go
ahead and step through the just flashed code. Typically, however, you will
not do so but rather stop this debug session again.

If you built your software for execution in RAM (see <<secRunInRAM>>) then
you don't need to flash at all. No matter what is currently flashed, just
start the corresponding debug configuration. The RAM is loaded with your
software and you can start it with the usual debugger commands to step and
run, etc.

If you end the debug session while the core(s) is running, i.e., the core
is not halted at a breakpoint, then the software in RAM even remains alive
and can be observed without a debugger connection. Only after the next
reset will the ROM software take effect again.

By the way, the debug sessions can also be found by clicking on the black
triangle next to the blue "bug" icon. The most recently used
configurations are listed in the menu. To see all of them or to
double-check their properties, click on "Debug Configurations..."
somewhere near the bottom of the list. In the dialog that opens, select
the desired configuration and start the debugger with a final click on the
button "Debug".
...
toggle between different boot headers. One build configuration supports
the FBL, the other one the BAF without the FBL.
+
