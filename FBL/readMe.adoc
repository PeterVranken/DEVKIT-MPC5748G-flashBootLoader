...
Flashing is done by CAN using two CCP. CCP requires two CAN messages the
CRO comand message and the DTO response message. The configuration of the
FBL requires the CAN device to use and the two CAN IDs for CRO and DTO.
The choice is made at compile-time by means of `#define` switches in
...
in the repoistory is the CAN bus, which is connected to the CAN
transceiver mounted on the DEVKIT-MP5748G and you can use the FBL out of
the box.
...
then you first need to modify the CAN driver configuration and enbale
...
The default configuration is 0x1234 of 4660. This address needs to be used
...
disable the authentication precedure.

== Supported CCP commands

Only a small subset of CCP comands is required for the tasks of an FBL.
...
[[tabSupportedCCP]]
.Supported CCP commands
[options="header",cols="^10e,^10e,80e", width="80%"]
|=======
|CCP |Code |Description

|CONNECT |0x01 | Establish a CCP connection with a particular device. The
station address is the argument of the comand. Pass 0x1234 to address to
our FBL.
...
reponse is sent only after erasure has completed. If the entire managed
...
- Erasure is always done for entire flash blocks and it is long lasting
...
of the computed digital signature to the FBL.
...
. Several `PROGAM_6` and or `PROGRAM` commands to write all wanted bytes
...
Any CCP command ends with a CAN response message. The most importent
...
and only later do the actual flash progamming.
...
regardless wether the same or other bytes of the page are programmed.
Before a page can be programmed again, the entire flash block containg the
...
the DEVKIT-MPC57548G, it is recommended to make use of the console
...
1. The uncommanded reset. Most of the time a power-on reset of the board.
In this case, the FBL checks the memories for a flashed, valid aplication.
...
for potential CCP `CONNECT` commads at its station address. A CCP client
...
+
Listing for CCP `COONECT` is an option only. The time designation can be
...
nearly idential to the use of the application without FBL. Please note
...
triggered by the FBL itself, on demand of a conencted CCP client.
+
In this case, the FBL checks the memories for a flashed, valid aplication.
...
Reset 2 is normally used only at the end of a successful CCP
communication sequence, which was initiated to flash an update of the
application onto the board: The new application is started with needing to
re-power the board.
...
flash ROM if an application is programmed in the DEVKIT-5748G. FBL and
...
header location for the application. If the boot header it placed at
address 0x00FD8000 then it is recognized by FBL and BAM. If the FBL is
erased then the application is still launched directly by the BAM. The
...
toggle between different bot headers. On build configuration supports the
FBL, the other one the BAM without the FBL.
+
...
CAN communication to progam the application.
+
